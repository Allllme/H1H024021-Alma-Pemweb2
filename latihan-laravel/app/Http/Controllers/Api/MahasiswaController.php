...
class MahasiswaController extends Controller
{
    ...
    public function perProgramStudi(ProgramStudi $programStudi, Request $request)
    {
        $perHalaman = min($request->integer('per_halaman', 10), 100);

        $daftarMahasiswa = $programStudi->mahasiswa()
            ->with('programStudi')
            ->orderBy('nama')
            ->paginate($perHalaman);

        return MahasiswaResource::collection($daftarMahasiswa);
    }
